...
> ✅ Le `config.json` livré est **complet** : toutes les sections (galerie, logiciels, e-books, musique, modules, concours, dons) **et** les 60 articles y sont renseignées. Le panneau les liste donc **toutes** dès l'import — si une liste est vide (e-books, concours), c'est simplement qu'aucun élément n'y est encore déclaré.

---

## 🚀 Déploiement léger — médias sur un 2ᵉ dépôt (CDN jsDelivr)

Pour garder **Cloudflare tout petit** (~3 Mo), les médias lourds sont **déportés** sur un second dépôt GitHub servi par le CDN gratuit **jsDelivr**. Deux livrables :

- **`truvector-site`** (~3 Mo) → **Cloudflare Pages**. Toute la structure : HTML, CSS, JS, `config.json`, mini-images d'interface. *Aucun MP3, aucune image de galerie, plus de `vault.json`.*
- **`truvector-assets`** (~33 Mo) → **nouveau dépôt GitHub public** `clode666/truvector-assets`. Contient `musique/` (12 MP3) et `galerie/` (`.webp` + `.png`).

**Le seul réglage** est dans `assets/js/assets.js` :

```js
window.TRU_ASSET_BASE = "https://cdn.jsdelivr.net/gh/clode666/truvector-assets@main";
```

Le lecteur audio, le mini-lecteur global et la galerie construisent leurs URLs à partir de cette base (`.../musique/<nom>.mp3`, `.../galerie/<slug>.webp|.png`). **Laisse la chaîne vide (`""`)** pour resservir les médias en local (mode monolithique) — le site fonctionne dans les deux cas.

**Mise en place (une fois) :**
1. Crée un dépôt GitHub **public** `truvector-assets`, pousse-y le contenu de `truvector-assets/`.
2. Vérifie/renseigne `TRU_ASSET_BASE` dans `assets/js/assets.js` (ton user + branche, **sans slash final**).
3. Déploie `truvector-site/` sur Cloudflare Pages (Git ou `wrangler pages deploy`).

**Mettre à jour un média :** remplace le fichier dans `truvector-assets` + `git push`. jsDelivr met `@main` en cache ~24 h–7 j ; pour forcer : purge `https://purge.jsdelivr.net/gh/clode666/truvector-assets@main/<chemin>`, ou crée un tag (`v1`, `v2`…) et pointe `TRU_ASSET_BASE` sur `@v1`.
...
Accessibles depuis les **onglets de la barre de navigation** (en haut à droite, sur toutes les pages) : *Accueil · Galerie · Logiciels · E-books · Musique · Articles · Modules · Concours · Soutenir*. L'onglet de la page courante est surligné ; sur mobile la rangée défile horizontalement. En **build PUBLIC** (celui livré), les sections sont ouvertes sans code ; en **build COFFRES**, chacune peut recevoir son propre code (dans `config.access.sections`, sauf `articles/` qui reste public) :
- `logiciels/` — catalogue de logiciels (pré-rempli, éditable dans `config.catalog.logiciels`).
- `ebooks/` — e-books et publications.
- `musique/` — **lecteur audio** ; fichiers dans `musique/assets/`, déclarés dans `config.catalog.musique`. La case **« Écouter en navigant »** (lecture en fond sur tout le site) transmet la piste et la position au **mini-lecteur global** (`assets/js/player.js`), qui reprend la musique **de page en page** en bas à droite (léger blanc au changement de page ; un clic sur ▶ peut être nécessaire après la 1re navigation, à cause de l'autoplay des navigateurs). **36 titres** livrés : **24 créations originales dans des styles radicalement différents** (rétrowave, lo-fi, chiptune, deep house, techno, trance, drum & bass, trap, ambient, trip-hop, funk, disco, reggae/dub, bossa nova, jazz, blues, rock, cinématique, baroque, valse, vaporwave, future bass, world/oriental, pizzicato) + les 10 pistes *romantico·geek* + les 2 démos d'origine. La playlist de la page lit les fichiers directement (`assets/<nom>.mp3`) ; pour en ajouter/retirer, édite `musique/assets/` + `config.catalog.musique` (et la liste dans `musique/index.html`).
...
- `modules/` — **modules interactifs** : petites applications autonomes, 100 % côté client (aucune dépendance externe), rangées dans `modules/apps/` et listées dans `config.catalog.modules`. **12 modules** livrés :
  - *Sécurité* — **TRU Keyforge** (mots de passe / passphrases / clés 256 bits), **TRU Hasher** (SHA-1 à SHA-512, texte ou fichier), **License Forge** (clés de licence de démo, format segmenté façon IronLock) ;
  - *Dev* — **Base64 · Hex Studio**, **Regex Lab**, **JSON Forge**, **Markdown Live** ;
  - *Outil* — **UXC Estimator** (entropie de Shannon + ratio gzip réel), **Cron Décodeur** ;
  - *Créatif* — **Aurora Weaver** (fond d'écran), **Color Aurora** (dégradés / palettes → CSS) ;
  - *Démo* — **GrowVault Lite**.
  Pour en ajouter un : dépose `modules/apps/<nom>.html`, ajoute une entrée `{ "title", "desc", "url": "apps/<nom>.html", "badge" }` dans `config.catalog.modules` (ou via l'admin), et une carte dans la grille de `modules/index.html`.
...
│   ├── index.html × 7         # 1 page par section (+ vault.json chiffré, utilisé en mode COFFRES)
│   ├── musique/assets/        # fichiers audio .mp3 (déclarés dans config.catalog.musique)
│   ├── modules/apps/          # 12 modules interactifs autonomes (keyforge, hasher, regex-lab, …)
│   └── articles/posts/        # 60 articles publics (blog indexable)
├── admin/
...
│   ├── js/player.js           # mini-lecteur global — musique sur toutes les pages (sauf page Musique)
│   ├── js/nav.js              # onglets de sections DIRECTS dans la barre de nav (v4)
...
| Ajouter un logiciel / e-book | `config.catalog.<nom>` (tableau d'items) |
| Ajouter un module interactif | `modules/apps/<nom>.html` + entrée `config.catalog.modules` + carte dans `modules/index.html` |
...
| Musique sur tout le site | page Musique → cocher **« Écouter en navigant »** |
| Onglets de navigation | `assets/js/nav.js` (rangée d'onglets, générée sur toutes les pages) |
