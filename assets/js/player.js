...
(function(){
  ...
  tracksThen(function(tracks){
    ...
    var audio=new Audio(); audio.preload='auto'; audio.volume=vol; audio.src=(window.TRU_ASSET_BASE?window.TRU_ASSET_BASE+'/musique/':root+'musique/assets/')+tracks[i].src;
    ...
    function go(idx,play){ i=(idx+tracks.length)%tracks.length; audio.src=(window.TRU_ASSET_BASE?window.TRU_ASSET_BASE+'/musique/':root+'musique/assets/')+tracks[i].src; set('tv_music_i',String(i)); label(); if(play) audio.play().catch(function(){}); }
